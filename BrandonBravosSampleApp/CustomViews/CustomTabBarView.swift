...
class CustomTabBarView: UIView {
    ...
    private var customTabBarBackgroundColor: UIColor = .white
    
    private var customTabBarIconColor: UIColor = .darkGray
    
    private var customTabBarSelectedIconColor: UIColor = #colorLiteral(red: 0.4392, green: 0.6275, blue: 0.6471, alpha: 1) /* #70a0a5 */
    ...
    private func currentPathSelected(indexPath: Int){
        guard let tabToActivate = self.subviews[indexPath] as? CustomTabView else {
            print("CustomTabBarView: cant downcast to CustomTabView")
            return
        }
    ...
    func activateTab(tab: Int) {
        ...
        guard let tabToActivate = self.subviews[tab] as? CustomTabView else {
            print("CustomTabBarView: cant downcast to CustomTabView")
            return
        }
        ...
        DispatchQueue.main.async {
            ...
            // here is where we want to do animations for our tabs.
                ...
                tabToActivate.itemIconView.tintColor = self.customTabBarSelectedIconColor
    ...
    func deactivateTab(tab: Int) {
        guard let inactiveTab = self.subviews[tab] as? CustomTabView else {
            print("CustomTabBarView: cant downcast to CustomTabView")
            return
        }
        ...
        DispatchQueue.main.async {
            UIView.animate(withDuration: 0.2, delay: 0.0, options: [.curveEaseIn, .allowUserInteraction], animations: {
            // here is where we want to do animations for our tabs.
                inactiveTab.itemIconView.tintColor = self.customTabBarIconColor
    ...
    func addTabsToView(menuItems: [CustomTabItem]) {
        ...
        for index in 0 ..< menuItems.count {
            ...
            let itemView = CustomTabView(item: menuItems[index]) // self.createTabItem(item: menuItems[i])
            itemView.itemIconView.tintColor = customTabBarIconColor
